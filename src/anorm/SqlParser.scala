...
object SqlParser {
  ...
  /**
   * get a specific typed RowParser
   * @param columnName
   * @param extractor
   * @return
   */
...
trait RowParser[+A] extends (Row => SqlResult[A]) { parent => // self type used for this aliasing
  ...
  /**
   * row combinator, glue two RowParser together
   * 
   * @param bParser
   * @return
   */
  def ~[B](bParser: RowParser[B]): RowParser[A ~ B] = RowParser { row =>
    parent(row) flatMap {
      a => bParser(row) map (new ~(a, _))
    }
  }
  ...
  /**
   * RowParser[A] => RowParser[Option[A]]
   * This a arity-0 function without () in definition, should be called using suffix notion.
   * @return
   */
  def ? : RowParser[Option[A]] = RowParser { row =>
    parent(row) match { // parent(row) returns a SqlResult, here matches the SqlResult
